...
var imagens = document.getElementById("informativos").children; //pega todos os nodos filhos da div #container, ou seja, nossa coleção de imagens
...
function agendarData() {
  ...
  var escritorio = document.getElementById("local").value;
  var numero = document.getElementById("estacao").value;
  ...
  if (nome == "") {
    ...
    cadastrado.innerHTML =
      "É obrigatório o preenchimento de todos os campos para agendar o seu retorno.";
  } else {
    var agendado = {
      nome: nome,
      data: dataFormatada,
...
function verificar(agendado, registro) {
  for (var verificando in agendado) {
    console.log(verificando + "=" + agendado[verificando]);
  }
  if (agendado.nome != "") {
    var cadastrado = document.getElementById("cadastrado");
    cadastrado.innerHTML =
      agendado.nome +
      " você agendou seu retorno ao trabalho para " +
      agendado.data +
      ", no escritório de " +
      agendado.escritorio +
      " e seu número na estação de trabalho é " +
      agendado.numero +
      ".";
  }
}